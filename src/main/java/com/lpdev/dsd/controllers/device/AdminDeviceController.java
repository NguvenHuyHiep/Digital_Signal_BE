...
public class AdminDeviceController {
  ...
  @GetMapping("/device-group/{id}/logs")
  protected ResponseEntity<BaseOutput<List<Device>>> getDeviceLogsByDeviceGroupId(
      @PathVariable("id") @NotBlank(message = DsdConstant.ERROR.REQUEST.INVALID_PATH_VARIABLE)
          Long id,
      @RequestParam(required = false, defaultValue = "UNDEFINED") DeviceStatus status,
    ...
    List<Device> devices =
        deviceService.getDeviceLogsByDeviceGroupId(id, status, startDate, endDate);
  ...
  @Operation(summary = "Delete a device by id", description = "Return deleted device")
  @ApiResponses(
      value = {
  ...
  protected ResponseEntity<Resource> exportDeviceStatus(
    ...
    DownloadFile downloadFile = deviceService.exportDeviceStatus(id, startDate, endDate);
