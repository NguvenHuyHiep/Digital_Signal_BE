...
public class AdminFileController {
  ...
  @GetMapping("")
  public ResponseEntity<BaseOutput<List<DsdFile>>> getByPaging(
  ...
  /* // TODO cannot put requeest body together with file's form data

    @PutMapping("/{fileId}")
    public ResponseEntity<BaseOutput<DsdFile>> update(
        @PathVariable @NotBlank(message = DsdConstant.ERROR.REQUEST.INVALID_PATH_VARIABLE)
            Long fileId,
        @RequestParam @NotBlank(message = DsdConstant.ERROR.REQUEST.INVALID_PARAM) MultipartFile file,
        @RequestBody @NotNull(message = DsdConstant.ERROR.REQUEST.INVALID_BODY) DsdFile dsdFile) {
      if (fileId == null
          || dsdFile == null
          || file == null
      ...
      dsdFile.setId(fileId);
      DsdFile updatedFile = dsdFileService.update(file, dsdFile);
      return ResponseEntity.ok(
          BaseOutput.<DsdFile>builder().message(HttpStatus.OK.toString()).data(updatedFile).build());
    }
  */
  ...
  public ResponseEntity<BaseOutput<DsdFile>> assignFileByPlaylists(
      @PathVariable("fileId") @NotBlank(message = DsdConstant.ERROR.REQUEST.INVALID_PATH_VARIABLE)
          Long fileId,
      @RequestBody @NotBlank(message = DsdConstant.ERROR.REQUEST.INVALID_PATH_VARIABLE)
  ...
  @Operation(summary = "Delete a file by path", description = "Return deleted file")
  @ApiResponses(
      value = {
        @ApiResponse(
            responseCode = "200",
  ...
  @Operation(summary = "Delete a file by path from source", description = "Return deleted file")
  @ApiResponses(
      value = {
        @ApiResponse(
            responseCode = "200",
  ...
  @DeleteMapping("/from-source")
  public ResponseEntity<BaseOutput<String>> deleteFromSource(
      @RequestParam @NotBlank(message = DsdConstant.ERROR.REQUEST.INVALID_PARAM) String path) {
    if (!StringUtils.hasText(path)) {
      BaseOutput<String> response =
          BaseOutput.<String>builder()
    ...
    dsdFileService.deleteFromSource(path);
  ...
  @Operation(summary = "Delete a file by path from database", description = "Return deleted file")
  @ApiResponses(
      value = {
        @ApiResponse(
            responseCode = "200",
  ...
  @DeleteMapping("/from-database")
  public ResponseEntity<BaseOutput<String>> deleteFromDatabase(
      @RequestParam @NotBlank(message = DsdConstant.ERROR.REQUEST.INVALID_PARAM) String path) {
    if (!StringUtils.hasText(path)) {
      BaseOutput<String> response =
          BaseOutput.<String>builder()
    ...
    dsdFileService.deleteFromDatabase(path);
